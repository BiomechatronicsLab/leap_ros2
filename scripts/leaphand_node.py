...
class DynamixelReaderNode(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "start_pos", [0] * 16  # Initial positions for the motors
        )
    ...
    def __del__(self):
        ...
        try:
            self.sync_write(
                self.motors, np.zeros(len(self.motors)), self.ADDR_TORQUE_ENABLE, 1
            )
    ...
    def initialize_gains(self):
        try:
            ...
            self.sync_write(
                self.motors, np.ones(len(self.motors)), self.ADDR_TORQUE_ENABLE, 1
            )

            # Enable position-current control mode and default parameters
            self.sync_write(
                self.motors, np.ones(len(self.motors)) * 3, self.ADDR_OPERATING_MODE, 1
            )
            self.sync_write(
                self.motors, np.ones(len(self.motors)) * self.kP, self.ADDR_P_GAIN, 2
            )  # P gain stiffness
            self.sync_write(
                [0, 4, 8], np.ones(3) * (self.kP * 0.75), self.ADDR_P_GAIN, 2
            )  # P gain stiffness for side to side should be a bit less
            self.sync_write(
                self.motors, np.ones(len(self.motors)) * self.kI, self.ADDR_I_GAIN, 2
            )  # I gain
            self.sync_write(
                self.motors, np.ones(len(self.motors)) * self.kD, self.ADDR_D_GAIN, 2
            )  # D gain damping
            self.sync_write(
                [0, 4, 8], np.ones(3) * (self.kD * 0.75), self.ADDR_D_GAIN, 2
            )  # D gain damping for side to side should be a bit less
            self.sync_write(
                self.motors,
                np.ones(len(self.motors)) * self.curr_lim,
                self.ADDR_CURRENT_LIMIT,
                2,
            )  # Max current limit
            self.sync_write(
                self.motors,
                self.degrees_to_ticks_list(self.curr_pos),
                self.ADDR_GOAL_POSITION,
                4,
